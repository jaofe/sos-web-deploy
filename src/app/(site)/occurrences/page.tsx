...
export default function Occurrences() {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/ocorrencia/${selectedOccurrenceId}/`, {
                method: "DELETE",
                headers: {
                                    ...
                                    {!occurrenceData || occurrenceData.length === 0 ? (
                                        ...
                                        occurrenceData.map((occurrence, index) => (
                                            <tr key={occurrence.id} className={`${index % 2 === 0 ? "bg-white" : "bg-[#E0E1EC]"}`}>
                                                <td className={`py-[12px] px-8 border-r border-l border-b border-[#272c6b] flex ${index === occurrenceData.length - 1 ? "rounded-bl-2xl" : ""}`}>
                                                    <span className="flex justify-center gap-2">
                                                        {icons[occurrence.tipo].icon}
                                                        {icons[occurrence.tipo].name}
                                                    </span>
                                                </td>
                                                <td className="py-[10px] px-8 border-r border-b border-[#272c6b] text-center">
                                                    {occurrence.bairro.length > 20 ? occurrence.bairro.substring(0, 20) + "..." : occurrence.bairro}
                                                </td>
                                                <td className="py-[10px] px-8 border-r border-b border-[#272c6b] text-center">{`${occurrence.latitude}, ${occurrence.longitude}`}</td>
                                                <td className="py-[10px] px-8 border-r border-b border-[#272c6b] text-center">
                                                ...
                                                <td className="py-[10px] px-8 border-r border-b border-[#272c6b] text-center">{occurrence.likes || 0}</td>
                                                <td className="py-[10px] px-8 border-r border-b border-[#272c6b] text-center">
                                                ...
                                                <td
                                                    className={`py-[12px] px-8 flex gap-2 border-b border-r border-[#272c6b] justify-center ${
                                                        index === occurrenceData.length - 1 ? "rounded-br-2xl" : ""
                                                    }`}
                                                >
